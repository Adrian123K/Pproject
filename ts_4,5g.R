...
ggPacf(diff(ts_skt4, differences = 1),lag=40)

##### skt 4g
auto.arima(ts_skt4,seasonal = F, stepwise=F, approximation = F)
ggtsdisplay(diff(ts_skt4,differences = 2))

skt4_fit <- arima(diff(ts_skt4),c(4,2,0))
dev.new()
...
kt4_fit <- arima(diff(ts_kt4,differences = 2),c(2,2,0),)
dev.new()
